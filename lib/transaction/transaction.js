...
function Transaction(serialized) {
  ...
  if (serialized) {
    if (serialized instanceof Transaction) {
      return Transaction.deepCopy(serialized);
...
/**
 * Create a deep copy of a tx
 */
Transaction.deepCopy = function(transaction) {
...
/**
 * Retrieve the normalized hash of the transaction
 * @return {Buffer}
 */
Object.defineProperty(Transaction.prototype, 'nid', {
  configurable: false,
  enumerable: true,
  get: function() {
    var txCopy = new Transaction(this);
    var inputs = txCopy.inputs.length;
    for (var i = 0; i < inputs; i++) {
      txCopy.inputs[i].setScript('');
    }
    var hash = txCopy._getHash();
    return new BufferReader(hash).readReverse().toString('hex');
  }
});
...
/**
 * Remove all inputs from the transaction.
 *
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.removeAllInputs = function() {
  this.inputs = [];
  this._inputAmount = undefined;
  this._updateChangeOutput();
  return this;
